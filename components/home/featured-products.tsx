...
const EXTENDED = [...products, ...products, ...products]; // 18 for infinite loop
const SET = products.length; // 6
const START = SET; // start in the middle copy
...
export function FeaturedProducts() {
  ...
  const go = async (dir: 1 | -1) => {
    ...
    await animate(x, -next * slotW, {
      duration: 0.42,
      ease: [0.4, 0, 0.2, 1],
    });

    // Cyclic snap-back (invisible because items repeat)
    let settled = next;
        ...
        <div ref={containerRef} className="overflow-hidden">
          <motion.div style={{ x }} className="flex">
            {EXTENDED.map((product, i) => (
              ...
              >
                <ProductCard product={product} index={i % SET} />
              </div>
            ))}
          </motion.div>
        </div>
