  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-center justify-center h-52 p-7 overflow-hidden">
        <span
          className="absolute font-serif text-[7rem] leading-none font-semibold select-none pointer-events-none text-white"
          style={{ opacity: 0.04 }}
        >
          F
        </span>
...
export function SuggestedProducts() {
  ...
  useEffect(() => {
    const calc = () => {
      ...
      const visible = mobile ? 1.4 : tablet ? 2.4 : 4;
      setSlotW(w / visible);
    };
    calc();
  ...
  const go = async (dir: 1 | -1) => {
    ...
    await animate(x, -next * slotW, { duration: 0.42, ease: [0.4, 0, 0.2, 1] });

    let settled = next;
    if (next >= SET * 2) settled = next - SET;
    else if (next < SET) settled = next + SET;
    ...
    <section className="relative w-full overflow-hidden py-24 lg:py-32 bg-black">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="flex items-end justify-between mb-12">
          ...
          >
            <p className="text-[10px] tracking-[0.45em] text-primary/60 uppercase mb-3">
              Selección para ti
            </p>
          ...
          >
            <Link
              href="/products"
              className="group hidden sm:inline-flex items-center gap-2.5 border border-primary/40 px-6 py-2.5 text-xs tracking-[0.2em] uppercase text-primary hover:bg-primary hover:text-black transition-all duration-300"
            >
              Ver todos
          ...
          <div ref={containerRef} className="overflow-hidden">
            <motion.div style={{ x }} className="flex items-stretch">
              {EXTENDED.map((product, i) => (
                <div
                  key={`${i}-${product.id}`}
                  style={{ width: slotW || "25%", flexShrink: 0, padding: "0 8px" }}
                >
